...
theorem get_up_low (i j k : Nat) (h : k < j) : (up j (@shift σ i)).get k = .var k := by
...
theorem get_up_high (i j k : Nat) (h : j ≤ k) : (up j (@shift σ i)).get k = .var (k + i) := by
...
theorem comp_eqv_comps (i j : Nat) : comp (up j (shift i)) (shift 1) ≈ (comps (up j (shift i)) 1 : Subst' σ) := by
  intros k
...
theorem applys_def (i j : Nat) : applys j i e = apply (up j (shift i)) e := by
  revert j
  -- Induction on `e`.
  apply @Expr.recOn _
...
@[simp]
theorem shift_expand : shift σ (i + 1 + 1) = comp (shift σ (i + 1)) (shift σ 1) := by
...
theorem apply_apply : t (s e) = (comp s t) e := by
  induction s using Quotient.inductionOn; rename_i s
  induction t using Quotient.inductionOn; rename_i t
...
@[simp]
theorem cons_comp : comp (cons e s) t = cons (s e) (comp s t) := by
  induction s using Quotient.inductionOn
...
theorem apply_zero_cons_shift_comp : cons (s (.var 0)) (comp (shift σ 1) s) = s := by
  induction s using Quotient.inductionOn
  rw [← shift_wrap, ← apply_wrap, ← comp_wrap, ← cons_wrap]
  apply Quotient.sound
  intros j
  rw [Subst'.apply_var]
  sorry
